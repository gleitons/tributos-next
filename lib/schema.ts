...
export const informacoes = sqliteTable('informacoes', {
    id: integer('id').primaryKey({ autoIncrement: true }),
    ...
    email: text('email'),
    endereco: text('endereco'),
    numero: text('numero'), // Keeping as text to be safe with "326" vs 326
    bairro: text('bairro'),
    cep: text('cep'),
    cidade: text('cidade'),
    estado: text('estado'),
    dataAbertura: text('data_abertura'),
    senhaSiare: text('senha_siare'),
    senhaGov: text('senha_gov'),
    senhaNotaFiscal: text('senha_nota_fiscal'),
    codigoSimples: text('codigo_simples'),
    titulo: text('titulo'),
    situacao: text('situacao'),
    mei: text('mei'),
    outrasInformacoes: text('outras_informacoes'),
    atividadePrincipal: text('atividade_principal'),
    atividadeSecundaria: text('atividade_secundaria'),
    regularize: text('regularize'),
    criadoEm: text('criado_em').default(sql`CURRENT_TIMESTAMP`),
});
